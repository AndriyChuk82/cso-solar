...
function calculateProposalTotals(proposal: Proposal): Proposal {
  ...
  const sellerId = proposal.seller?.id || (proposal as any).sellerId || 'fop_pastushok';
    ...
    (set, get) => ({
      ...
      loadProducts: async () => {
        ...
        try {
          ...
          if (allData) {
            const { rates: serverRates, products: rawProducts, customMaterials } = allData;
            
            // Автоматично оновлюємо курси з Hoverla напряму при запуск додатка
            let rates = serverRates;
            try {
              console.log('🔄 Автоматичне оновлення курсів з Hoverla при завантаженні...');
              const freshRates = await fetchRates();
              if (freshRates && freshRates.usd && freshRates.eur) {
                rates = { usd: freshRates.usd, eur: freshRates.eur };
                console.log('✅ Автоматично отримано свіжі курси з Hoverla:', rates);
              } else if (freshRates && freshRates.error) {
                toast.error(freshRates.error);
              }
            } catch (rateErr) {
              console.warn('⚠️ Помилка автоматичного оновлення курсів при старті:', rateErr);
            }
