...
export default function PaymentForm({ plots, onClose }: PaymentFormProps) {
  const { createPayment } = useFinanceStore()
  const [formData, setFormData] = useState({
    plot_id: plots.length > 0 ? plots[0].id : '',
    payment_date: new Date().toISOString().split('T')[0],
    payment_type: 'money' as ChargeType,
    amount: 0,
  ...
  useEffect(() => {
    setFormData(prev => ({
      ...prev,
      unit: CHARGE_TYPE_UNITS[prev.payment_type]
    }))
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 modal-overlay p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md modal-content">
        <div className="flex items-center justify-between p-4 border-b modal-header">
          <h2 className="text-lg font-semibold">Додати оплату</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="form-group">
              ...
              <select
                ...
                onChange={e => setFormData({ ...formData, payment_type: e.target.value as ChargeType })}
                className="w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
