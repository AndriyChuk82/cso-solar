          ...
          <div className="flex items-center justify-between h-14">
            ...
            <div className="flex items-center gap-3">
              <nav className="hidden md:flex items-center gap-1.5">
                {(user?.role?.toLowerCase().includes('admin') || user?.module_access?.includes('proposals') || user?.module_access?.includes('кп') || user?.module_access?.includes('комперційні')) && (
                  <a
                    href="/proposals/"
                ...
                {(user?.role?.toLowerCase().includes('admin') || user?.module_access?.includes('warehouse') || user?.module_access?.includes('склад')) && (
                  <a
                    href="/warehouse/"
                    className="px-3 py-2 text-sm font-medium text-gray-700 dark:text-neutral-300 hover:text-primary hover:bg-gray-100 dark:hover:bg-neutral-700 rounded-md transition"
                  >
                ...
                {(user?.role?.toLowerCase().includes('admin') || user?.module_access?.includes('projects') || user?.module_access?.includes('проєкти') || user?.module_access?.includes('проекти')) && (
                  <a
                    href="/projects/"
                    className="px-3 py-2 text-sm font-medium text-gray-700 dark:text-neutral-300 hover:text-primary hover:bg-gray-100 dark:hover:bg-neutral-700 rounded-md transition"
                  >
                ...
                {(user?.role?.toLowerCase().includes('admin') || user?.module_access?.includes('gt') || user?.module_access?.includes('зелений тариф') || user?.module_access?.includes('зт')) && (
                  <a
                    href="/green-tariff/"
