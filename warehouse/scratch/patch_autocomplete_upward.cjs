...
files.forEach(filepath => {
  ...
  // Normally: top-11 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg shadow-xl max-h-80 overflow-y-auto z-50 text-xs divide-y divide-[var(--border)]
  // We want to replace top-11 with dynamic template string
  
  const targetDropdown = 'className="absolute left-2 right-2 top-11 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg shadow-xl max-h-80';
  const replacementDropdown = 'className={`absolute left-2 right-2 ${isLastRows ? \'bottom-full mb-1\' : \'top-11\'} bg-[var(--bg-card)] border border-[var(--border)] rounded-lg shadow-xl max-h-80';
