  ...
  return (
    <div className="proposal-print-container">
      ...
      <div className="no-print" style={{ marginBottom: '20px', display: 'flex', gap: '10px' }}>
        ...
        <button className="btn btn-primary" onClick={() => window.print()}>🖨️ Друкувати / Більше</button>
      </div>
      ...
      <div className="print-header">
        ...
        <div className="pro-info">
          <div className="pro-number">КП №{proposal.id.slice(-6).toUpperCase()}</div>
        ...
        <tbody>
          {proposal.items.map((item, idx) => (
            <tr key={idx}>
              ...
              <td style={{ textAlign: 'right' }}>{new Intl.NumberFormat('uk-UA').format(item.price)}</td>
              <td style={{ textAlign: 'right' }}>{new Intl.NumberFormat('uk-UA').format(item.total)}</td>
            </tr>
          ))}
        </tbody>
      </table>
