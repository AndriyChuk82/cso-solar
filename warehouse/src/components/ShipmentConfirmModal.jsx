  ...
  async function handleSubmit(e) {
    ...
    try {
      if (isBatchMode) {
        ...
        const res = await confirmShipmentDispatch(shipment.id, { ttn, carrier }, user);
        if (res.success) {
          showToast('Відправку та списання товарів підтверджено', 'success');
          onSuccess();
        } else {
          showToast('Помилка підтвердження відправки', 'error');
        }
      }
    } catch (err) {
      console.error('Dispatch confirmation failed:', err);
      showToast(err.message || 'Помилка проведення відправки', 'error');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="w-full max-w-md rounded-2xl bg-white dark:bg-neutral-800 p-6 shadow-xl border border-gray-100 dark:border-neutral-700">
        <div className="flex items-center justify-between mb-4 pb-3 border-b border-gray-100 dark:border-neutral-700">
          <h3 className="text-lg font-bold text-gray-900 dark:text-white flex items-center gap-2">
            🚚 {isBatchMode ? `Масове підтвердження відправки (${selectedShipments.length})` : 'Підтвердити відправку & ТТН'}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {!isBatchMode && shipment && (
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 rounded-xl text-sm space-y-1">
              ...
              <div><strong>Адреса:</strong> {shipment.shipping_address || 'Не вказано'}</div>
              <div className="text-xs text-amber-700 dark:text-amber-400 pt-1">
                ⚠️ Увага: При підтвердженні товар буде остаточно списано з залишків складу!
              </div>
            </div>
          )}
          ...
          {isBatchMode && (
            <div className="p-3 bg-amber-50 dark:bg-amber-900/20 text-amber-800 dark:text-amber-300 rounded-xl text-sm space-y-1">
              <div>Ви обрали <strong>{selectedShipments.length}</strong> відправлень для масової відправки.</div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Нова Пошта, Укрпошта тощо..."
              required
            />
          </div>
          ...
          <div className="flex items-center justify-end gap-3 pt-3 border-t border-gray-100 dark:border-neutral-700">
            ...
            <button
              type="submit"
              disabled={loading}
              className="px-5 py-2 text-sm font-semibold text-white bg-amber-600 hover:bg-amber-700 rounded-xl shadow-md transition-all flex items-center gap-2 disabled:opacity-50"
            >
              {loading ? 'Проведення...' : '🚀 Підтвердити та списати товар'}
            </button>
          </div>
        </form>
      </div>
