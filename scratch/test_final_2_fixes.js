async function testFinal2Fixes() {
  ...
  function extractProductInfoForPrimary(name) {
    ...
    // 4. Вилучаємо великі комплекти обладнання
  ...
  // Test BOS-G
  const bosName = "Deye BOS-G 5.1 PRO";
  const bosInfo = extractProductInfoForPrimary(bosName);
  const bosHMatch = findBestMatchForPrimary(bosInfo, heliusProducts);
  console.log("BOS-G Helius Match:", bosHMatch ? `${bosHMatch.model.substring(0, 50)} -> ${bosHMatch.price} $` : 'None');

  // Test Huawei
  const huName = "Huawei SUN2000-30KTL-M3";
  const huInfo = extractProductInfoForPrimary(huName);
  const huPEMatch = findBestMatchForPrimary(huInfo, peProducts);
  console.log("Huawei PE Match:", huPEMatch ? `${huPEMatch.model.substring(0, 50)} -> ${huPEMatch.price} $` : 'None');
}

testFinal2Fixes().catch(console.error);
