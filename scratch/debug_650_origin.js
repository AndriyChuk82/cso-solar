async function debug650Origin() {
  const sidHelius = '1HANizDH1A5Vd_aNU9Xu6mgRXJx2Yqb7mQz534CivyII';
  const sidPE = '1dXuNar4t3aemQSk5LnPOXxcB7DAqqEzraWEFASa2r4g';

  function parseCSVText(text) {
    const lines = [];
    let row = [];
    let cell = '';
    let inQuotes = false;
    for (let i = 0; i < text.length; i++) {
      const c = text[i];
      if (inQuotes) {
        if (c === '"') {
          if (text[i + 1] === '"') {
            cell += '"';
            i++;
          } else {
            inQuotes = false;
          }
        } else {
          cell += c;
        }
      } else {
        if (c === '"') {
          inQuotes = true;
        } else if (c === ',') {
          row.push(cell);
          cell = '';
        } else if (c === '\n' || c === '\r') {
          if (c === '\r' && text[i + 1] === '\n') {
            i++;
          }
          row.push(cell);
          lines.push(row);
          row = [];
          cell = '';
        } else {
          cell += c;
        }
      }
    }
    if (row.length > 0 || cell) {
      row.push(cell);
      lines.push(row);
    }
    return lines;
  }

  function cleanStrForPrimary(s) {
    return (s || '').toLowerCase().replace(/[^a-z0-9]/gi, '');
  }

  function parsePriceValueForPrimary(str) {
    if (!str) return null;
    let s = String(str).trim();
    const low = s.toLowerCase();
    if (low.includes('рок') || low.includes('рік') || low.includes('гарант')) return null;
    if (low.includes('гот') || s.includes('/')) {
      const match = s.match(/[\d\s,.]+/);
      if (match) s = match[0];
    }
    s = s.replace(/[$€₴]|грн/gi, '').trim();
    s = s.replace(/\s/g, '').replace(',', '.');
    const val = parseFloat(s);
    return (isNaN(val) || val <= 0) ? null : val;
  }

  function extractProductInfoForPrimary(name) {
    if (!name) return null;
    const s = name.trim();
    const low = s.toLowerCase();
    if (low.includes('longi') || low.includes('ja solar') || low.includes('jasolar') || low.includes('панель') || low.includes('панелі')) return null;
    if (low.includes('240kwh') || low.includes('bos-b a3')) return null;
    if (low.includes('кабель')) return null;
    if (low.includes('комплект')) return null;
    if (low.includes('стійка') || low.includes('rack')) {
      if (low.includes('8') || low.includes('lrack')) return { type: 'rack', key: 'rack8' };
      if (low.includes('12') || low.includes('13') || low.includes('hrack')) return { type: 'rack', key: 'rack13' };
      return { type: 'rack', key: cleanStrForPrimary(s) };
    }
    if (low.includes('pdu2') || (low.includes('deye') && low.includes('bms'))) return { type: 'bms', key: 'deye_bms_pdu2' };

    let m = low.match(/sun[-_\s]*0?(\d+k?)[-_\s]*(sg\d+)?[-_\s]*([lhb]p\d+)?/i);
    if (m) {
      const kw = parseInt(m[1], 10);
      const sg = (m[2] || '').toLowerCase();
      const p = (m[3] || '').toLowerCase();
      let keyParts = ['sun', kw + 'k'];
      if (sg) keyParts.push(sg);
      if (p) keyParts.push(p);
      return { type: 'inverter', kw, sg, p, key: keyParts.join('_') };
    }
    return { type: 'other', key: cleanStrForPrimary(s) };
  }

  console.log("=== SEARCHING HELIUS FOR 650 OR SUN-10K ===");
  const urlH = `https://docs.google.com/spreadsheets/d/${sidHelius}/gviz/tq?tqx=out:csv&sheet=_Прайс_Хеліус`;
  const resH = await fetch(urlH);
  const textH = await resH.text();
  const rowsH = parseCSVText(textH);
  rowsH.forEach((r, idx) => {
    const lineStr = r.join(' ');
    if (lineStr.includes('650') || lineStr.toLowerCase().includes('10k') || lineStr.toLowerCase().includes('sg05')) {
      console.log(`[Helius Row ${idx+1}] ${r.slice(0, 7).join(' | ')}`);
    }
  });

  console.log("\n=== SEARCHING PE FOR 650 OR SUN-10K ===");
  const targetSheets = ['Гібридні інвертори', 'Мережеві інвертори', 'АКБ'];
  for (let sheetName of targetSheets) {
    const urlP = `https://docs.google.com/spreadsheets/d/${sidPE}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}`;
    const resP = await fetch(urlP);
    const textP = await resP.text();
    const rowsP = parseCSVText(textP);
    rowsP.forEach((r, idx) => {
      const lineStr = r.join(' ');
      if (lineStr.includes('650') || lineStr.toLowerCase().includes('10k') || lineStr.toLowerCase().includes('sg05')) {
        console.log(`[PE ${sheetName} Row ${idx+1}] ${r.slice(0, 8).join(' | ')}`);
      }
    });
  }
}

debug650Origin().catch(console.error);
