async function debugRackPE() {
  ...
  for (let sheetName of targetSheets) {
    ...
    rows.forEach((row, idx) => {
      ...
      if (lineStr.toLowerCase().includes('стійка') || lineStr.toLowerCase().includes('rack')) {
        console.log(`[PE Sheet ${sheetName} Row ${idx+1}] ${row.slice(0, 5).join(' | ')}`);
      }
    });
  }
}

debugRackPE().catch(console.error);
