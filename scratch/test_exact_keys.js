async function testExactKeys() {
  ...
  function extractProductInfoForPrimary(name) {
    ...
    if (m) {
      ...
      // key includes sg if present to distinguish SG04 vs SG05, SG01 vs SG02
  ...
  function findBestMatchForPrimary(targetInfo, catalog) {
    ...
    // 2. Partial match without sg if exact sg match not found
  ...
  console.log("=== FINAL VERIFIED MATCHES ===");
...
testExactKeys().catch(console.error);
