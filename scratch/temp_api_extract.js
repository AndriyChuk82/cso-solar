...
function extractPanelPower(name: string): number | null {
  if (!name) return null;
...
function adjustSolarPanelPrice(name: string, price: number, category: string): number {
...
function cleanAndFormatProductName(name: string): string {
  ...
  // Split commonly glued technical specs at the end of formatting
...
function mergeSupplierProducts(
...
function searchProducts(products: Product[], query: string): Product[] {
  ...
  .map(item => item.product);
    ...
    module.exports = { extractModelCode, cleanAndFormatProductName };
