async function testFallbackFix() {
  ...
  function findBestMatchForPrimary(targetInfo, catalog) {
    ...
    if (targetInfo.type === 'inverter') {
      if (targetInfo.sg) {
        match = catalog.find(item => item.info.type === 'inverter' && item.info.kw === targetInfo.kw && item.info.sg === targetInfo.sg && item.info.p === targetInfo.p);
        if (match) return match;
        return null; // Do not fallback to G04 or different series when SG series is specified!
      }
    }
  ...
  console.log("PE Match:", peMatch ? `Sheet "${peMatch.sheetName}" Model "${peMatch.model}" -> ${peMatch.price} $` : 'Not Found in PE (Out of stock)');
}
