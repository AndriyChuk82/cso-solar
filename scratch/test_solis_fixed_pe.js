async function testSolisFixedPE() {
  ...
  function extractProductInfoForPrimary(name) {
    ...
    if (low.includes('solis')) {
      if (low.includes('gc30k') || low.includes('gc3p30k') || low.includes('s5-gc30k') || low.includes('3p-30k') || low.includes('3p-30k')) {
  ...
  for (let sheetName of targetSheets) {
    ...
    for (let i = 1; i < rowsP.length; i++) {
      ...
      if (model && price) {
        ...
        if (info) peProducts.push({ model, price, info, sheetName });
      }
    }
  }
  ...
  console.log("PE Match:", peMatch ? `Sheet "${peMatch.sheetName}" Model "${peMatch.model}" -> ${peMatch.price} $` : 'None');
}

testSolisFixedPE().catch(console.error);
