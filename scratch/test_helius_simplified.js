...
async function testHeliusSimplified() {
  const url = `https://docs.google.com/spreadsheets/d/${HELIUS_SPREADSHEET_ID}/export?format=csv&gid=${HELIUS_GID}`;
  ...
  console.log(`Simplified Helius items extracted: ${products.length}`);
  ...
  testItems.forEach((item, idx) => {
    ...
    if (m) matched++;
    console.log(`[${idx+2}] "${item}" -> ${m ? `✅ ${m.price} $ ("${m.model.substring(0, 40)}")` : '❌ Not found'}`);
  });

  console.log(`Matched Helius: ${matched}/${testItems.length}`);
}

testHeliusSimplified().catch(console.error);
